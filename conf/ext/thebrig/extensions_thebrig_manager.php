...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr><td class="tabnavtbl">
		<ul id="tabnav2">
			<li class="tabinact"><a href="extensions_thebrig_tarballs.php"><span><?=_THEBRIG_UPDATER;?></span></a></li>
			<li class="tabact"><a href="extensions_thebrig_ports.php" title="<?=gettext("Reload page");?>"><span><?=_THEBRIG_PORTS;?></span></a></li>
			<li class="tabinact">
				<a href="extensions_thebrig_tools.php"><span><?=_THEBRIG_MANAGER;?></span></a>
			</li>
		</ul>
	</td></tr>

	<tr><td class="tabcont">
		<form action="extensions_thebrig_ports.php" method="post" name="iform" id="iform" onsubmit="return checkBeforeSubmit();">
		...
		elseif ( $brig_update_ready == 1 ){
			...
			exec ( "fetch -o " . $brig_port_db . "latest.ssl http://portsnap.freebsd.org/latest.ssl");
			...
			$most_date= exec( "date -j -r `" . $brig_root . "conf/bin/openssl rsautl -pubin -inkey "
					. $brig_port_db . "pub.ssl -verify < "
					. $brig_port_db . "latest.ssl | cut -f 2 -d '|'`");
			exec ("rm " . $brig_root . "conf/db/ports/latest.ssl"); 	// Get rid of the latest tag
			...
			html_text($confconv, gettext("Current Status"),"The latest snapshot on the FTP server is dated: " . $most_date . "<br /><br />Your copy of the snapshot was released: " . $tagdate . "<br /><br />The last time a snapshot was applied to the tree was: " . $extractdate );
			// We have never gotten a tag before - we need to fetch and extract a copy first
			if ( $tagdate === "Never"){?>
				<tr>
				<td width="22%" valign="top" class="vncell">Fetch and Extract the tree&nbsp;</td>
				<td width="78%" class="vtable">
				<?=gettext("The first step to using a central ports tree is to fetch and extract the snapshot of the ports tree. <br /><b>This step should only need to be done the very first time, as it will overwrite anything in the ports tree directory!</b>");?><br />
				<div id="submit_x">
					<input id="finstall" name="port_op" type="submit" class="formbtn" value="<?=gettext("Fetch & Install");?>" onClick="return conf_handler();"/><br />
				</div></td></tr>
			<?php }	
			else {
				// We have tag meaning we have downloaded & extracted a copy of the tree before - now we just want to update it.?>
			<tr>
			<td width="22%" valign="top" class="vncell">Fetch and Update the tree&nbsp;</td>
			<td width="78%" class="vtable">
			<?=gettext("Click below to perform an 'on-demand' fetch and update of the exising ports tree.");?><br />
				<div id="submit_x">
					<input id="fupdate" name="port_op" type="submit" class="formbtn" value="<?=gettext("Fetch & Update");?>" onClick="return conf_handler();"/><br />
				</div>
			</td>
			</tr>
			...
			<td width="78%" class="vtable">
			<?=gettext("Click below to update of the exising ports tree using the most recently downloaded snapshot.");?><br />
				<div id="submit_x">
					<input id="update" name="port_op" type="submit" class="formbtn" value="<?=gettext("Update");?>" onClick="return conf_handler();" /><br />
				</div>
			</td>
			</tr>
		<?php 
			...
			html_titleline(gettext("Jails"));}?>
				<tr><td width="15%" valign="top" class="vncell"><?=gettext("Ports Tree Enabling");?></td>
						<td width="85%" class="vtable">Please choose which of the following jails should have access to the shared ports tree:<br /><br />
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr>
									...
									<td width="19%" class="listhdrr"><?=htmlspecialchars(gettext("Path"));?></td>
									<td width="19%" class="listhdrr"><?=gettext("Description");?></td>
									<td width="5%" class="list"></td>
								</tr>
							<?php $k = 0; for( $k; $k < count ( $a_jail ) ; $k ++ ):?>
								<tr>
									<td class="<?=$enable?"listr":"listrd";?>"><input type="checkbox" name="formJails[]" value=<?php echo "{$a_jail[$k]['uuid']}";?> <?php /* This ugliness is a hack based on deselecting a jail will show checked after submitting.*/echo ( $_POST ) ? ( (FALSE !== ($cnid_test = array_search($a_jail[$k]['uuid'], $formjails )) ) ? "checked" :  "unchecked" ) : ( ( isset($a_jail[$k]['ports']) ) ? "checked" :  "unchecked" ); ?>>&nbsp;</td>
							...
							<br><br>
							<b><?=gettext("Please note: ")?></b><?=gettext("Selecting a jail to have access to the portstree will result in a non-standard make.conf")?>  
								<tr>
						<td width="15%" valign="top" class="vncell"><?=gettext("Cronjob");?></td>
						<td width="85%" class="vtable">
							<input name="portscron" type="checkbox" id="portscron" value="yes" <?php if (!empty($pconfig['portscron'])) echo "checked=\"checked\""; ?> /><?=_THEBRIG_PORT_CRON?><br />
							</td>
					</tr>

				</table>
				<div id="submit_x">
					<input id="save" name="save" type="submit" class="formbtn" value="<?=gettext("Save");?>" onClick="return conf_handler();"/>
				</div>
		<?php }?>
